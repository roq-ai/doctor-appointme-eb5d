...
export function DoctorListPage(props: DoctorListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getDoctors({
        relations: ['organization', 'appointment.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'specialty.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DoctorInterface>>(
    () => `/doctors?params=${JSON.stringify(params)}`,
    fetcher,
  );
